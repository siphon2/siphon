...
*     ---------------------------------------------------------------------------
*     
*     LIST A DIRECTORY WITH OPTIONS SPECIFIED IN ARGUMENTS
*     
*/
...
$arguments = [
    ...
    "path" => isset($_COOKIE['path']) ? $_COOKIE['path'] : __DIR__,                         // Path to be listed
    "page" => isset($_COOKIE['page']) ? (int)$_COOKIE['page'] : 1,                          // Page to be returned
    "limit" => isset($_COOKIE['limit']) ? (int)$_COOKIE['limit'] : 10,                      // Set number of result per page
    "details" => isset($_COOKIE['details']) ? ($_COOKIE['details'] === 'true') : false      // Get details of the listed items i.e size, mime and type etc
];
    ...
    else
    {
        ...
        while (($file = readdir($handle)) !== false && $total_count < $limit)
        {
            if ($file != '.' && $file != '..')
            {
                ...
                if ($details)
                {
                    $stat = (object)stat($fullPath);
                    $info['name'] = $file;
                    $info['type'] = is_link($fullPath) ? 'symlink' : (is_file($fullPath) ? 'file' : (is_dir($fullPath) ? 'dir' : 'unknown'));
                    if ($info['type'] == 'symlink')
                    {
                        $info['dest'] = readlink($fullPath);
                    }
                    $info['mime'] = mime_content_type($fullPath);
                    if ($info['mime'] != 'directory')
                    {
                        $info['exec'] = is_executable($fullPath);
                    }
                    $info['size'] = $stat->size;
